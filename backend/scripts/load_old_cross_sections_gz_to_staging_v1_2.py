#!/usr/bin/env python3
"""
scripts/load_old_cross_sections_gz_to_staging_v1_2.py

Plan-basierter OLD-Loader (old_cross_sections/*.csv.gz) -> staging.stg_old_mq_hr
...
- Expandiert old_cross_sections glob
...
class OldMqRow:
    ...
    q_kfz_mq_hr: Optional[int]
    v_kfz_mq_hr: Optional[int]
    q_pkw_mq_hr: Optional[int]
    v_pkw_mq_hr: Optional[int]
    q_lkw_mq_hr: Optional[int]
    v_lkw_mq_hr: Optional[int]
    source_file: str
    ingested_at: datetime


def _row_score(r: OldMqRow) -> int:
    fields = [
        r.qualitaet,
        r.q_kfz_mq_hr,
        r.v_kfz_mq_hr,
...
def replace_month_slice(cur: psycopg.Cursor, start: date, end: date) -> int:
    """Löscht alle bestehenden Zeilen des Monats aus staging.stg_old_mq_hr (Idempotenz)."""
    cur.execute(
        "DELETE FROM staging.stg_old_mq_hr WHERE tag >= %s AND tag < %s;", (start, end)
    )
    return cur.rowcount


def copy_rows(cur: psycopg.Cursor, rows: Iterable[OldMqRow]) -> int:
    """Schreibt OldMqRow-Objekte per COPY-Protokoll in staging.stg_old_mq_hr."""
    sql = """
    COPY staging.stg_old_mq_hr (
      mq_name, tag, stunde, qualitaet,
      q_kfz_mq_hr, v_kfz_mq_hr,
      q_pkw_mq_hr, v_pkw_mq_hr,
...
def iter_rows_from_gz(gz_path: Path, *, start: date, end: date) -> Iterator[OldMqRow]:
    """Liest Querschnitt-Messwerte aus einer komprimierten CSV.GZ-Datei (OLD-Format).
    ...
    Dedup: Pro (mq_name, Datum, Stunde) wird die vollständigere Zeile behalten.
    """
    ...
    with gzip.open(gz_path, "rt", encoding="utf-8", errors="replace", newline="") as f:
        ...
        if i_tag is None or i_hr is None:
            raise RuntimeError(
                f"Pflichtspalten nicht gefunden in {gz_path.name}. Headers={headers}"
            )

        i_name = _idx(idx_map, "mq_name", "mqname", "mq", "mq_kurzname")

        i_q = _idx(idx_map, "qualitaet", "qualität", "quality")
        i_qk = _idx(idx_map, "q_kfz_mq_hr", "qkfz_mq_hr", "q_kfz")
        i_vk = _idx(idx_map, "v_kfz_mq_hr", "vkfz_mq_hr", "v_kfz")
        i_qp = _idx(idx_map, "q_pkw_mq_hr", "qpkw_mq_hr", "q_pkw")
        i_vp = _idx(idx_map, "v_pkw_mq_hr", "vpkw_mq_hr", "v_pkw")
        i_ql = _idx(idx_map, "q_lkw_mq_hr", "qlkw_mq_hr", "q_lkw")
        i_vl = _idx(idx_map, "v_lkw_mq_hr", "vlkw_mq_hr", "v_lkw")

        reader = csv.reader(f, delimiter=delim)

        keep: Dict[Tuple[Optional[str], date, int], OldMqRow] = {}
        ...
        for row in reader:
            ...
            cand = OldMqRow(
                ...
                q_kfz_mq_hr=_parse_int(get(i_qk)),
                v_kfz_mq_hr=_parse_int(get(i_vk)),
                q_pkw_mq_hr=_parse_int(get(i_qp)),
                v_pkw_mq_hr=_parse_int(get(i_vp)),
                q_lkw_mq_hr=_parse_int(get(i_ql)),
                v_lkw_mq_hr=_parse_int(get(i_vl)),
                source_file=str(gz_path),
                ingested_at=ingested_at,
            )

            k = (cand.mq_name, cand.tag, cand.stunde)
            prev = keep.get(k)
            if prev is None or _row_score(cand) >= _row_score(prev):
                keep[k] = cand

        for k in sorted(keep.keys(), key=lambda x: (x[1], x[2], x[0] or "")):
            yield keep[k]
...
def main() -> None:
    ...
    try:
        grand_total = 0
        for plan in plans:
            if not plan.old_enabled or plan.old_cross_sections_glob_abs is None:
                print(f"[SKIP] month={plan.month_key} old_cross_sections deaktiviert")
                continue

            start, end = month_key_to_range(plan.month_key)
            files = expand_glob(plan.old_cross_sections_glob_abs)
            if args.max_files is not None:
                files = files[: args.max_files]

            if not files:
                print(
                    f"[SKIP] month={plan.month_key} keine Dateien gefunden: {plan.old_cross_sections_glob_abs}"
                )
                continue
            ...
            grand_total += total
            print(
                f"[OK] month={plan.month_key} inserted={total} files={len(files)} -> staging.stg_old_mq_hr"
            )
